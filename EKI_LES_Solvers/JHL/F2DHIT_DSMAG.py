...
from smag_eddy_viscosity import *
...
# cs = 0.2#cs# Smag coefficients
    ...
    # cs = 0.2
    ev_coeff = (cs[0]*Delta)**5 # Five for Leith / Four for SMAG
    ev_coeff_gpu = gpuarray.to_gpu(np.reshape(ev_coeff,1,order='F'))
    CB_gpu = gpuarray.to_gpu(np.reshape(cs[1],1,order='F'))
    ...
    for it in range(maxit):
        if it == 0:
            ...
            # Eddy viscosity
            vt    = np.zeros([1,1],dtype = np.complex128)
            vb    = np.zeros([1,1],dtype = np.complex128)
            ...
            Ksq4_gpu = gpuarray.to_gpu(np.reshape(Ksq4,NX*(NX),order='F'))
            ...
            vt_gpu    = gpuarray.to_gpu(np.reshape(vt,1,order='F'))
            vb_gpu    = gpuarray.to_gpu(np.reshape(vb,1,order='F'))
            ...
            psi_gpu = gpuarray.to_gpu(np.reshape(s,NX*(NX),order='F'))
            diffu_gpu = gpuarray.to_gpu(np.reshape(s,NX*(NX),order='F'))
            hyperdiffu_hat_gpu = gpuarray.to_gpu(np.reshape(s,NX*(NX),order='F'))
            hyperdiffu_gpu = gpuarray.to_gpu(np.reshape(s,NX*(NX),order='F'))
            Fsmag = gpuarray.to_gpu(np.reshape(s,NX*(NX),order='F'))
            vb1_gpu = gpuarray.to_gpu(np.reshape(s,NX*(NX),order='F'))
            vb2_gpu = gpuarray.to_gpu(np.reshape(s,NX*(NX),order='F'))
        ...
        SMAG(vt_gpu, vb_gpu, kx_gpu, w1_hat_gpu, w1x_gpu, w1y_gpu, psiCurrent_hat_gpu, diffu_hat_gpu, NX, TPB, NB, HNX, HNB, Delta, plan, ev_coeff_gpu,
        s1_hat_gpu, s2_hat_gpu, s1_gpu, s2_gpu, s_gpu, hyperdiffu_hat_gpu, diffu_gpu, hyperdiffu_gpu, Fsmag, psi_gpu, meanvb1_gpu, meanvb2_gpu
        , CB_gpu, vb1_gpu, vb2_gpu)

        # vt = vt_gpu.get()
        # print(vt)

        # Construct RHS
        RHS_ker(convec_hat_gpu, diffu_hat_gpu, vb_gpu, w1_hat_gpu, RHS_gpu, block=(TPB,1,1), grid=(NB,NX,1)) 
        ...
        LHS_ker(kx_gpu, RHS_gpu, vb_gpu, w1_hat_gpu, block=(TPB,1,1), grid=(HNB,HNX,1))


        # Hyper diffusion
        hyperFilter_ker(w1_hat_gpu, vt_gpu, Ksq4_gpu, block=(TPB,1,1), grid=(HNB,HNX,1))
    ...
    vb = np.real(vb_gpu.get())
    vt = np.real(vt_gpu.get())
    # savemat('w1.mat',dict([('w1Python', u0),('u1Python', u1),('v1Python', v1),('slnW',slnW),('vbP',vb),('vtP',vt)]))
    print(time)
    ...
    # X.reshape([NX*NX, 1])
    # Y.reshape([NX*NX, 1])
    t = np.linspace(0, time-NSAVE*dt, num=NNSAVE)
    print(np.shape(t))
