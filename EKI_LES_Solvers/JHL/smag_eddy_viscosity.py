...
def SMAG(vt_gpu, vb_gpu, kx_gpu, w1_hat_gpu, w1x_gpu, w1y_gpu, psiCurrent_hat_gpu, diffu_hat_gpu, NX, TPB, NB, HNX, HNB, Delta, plan, ev_coeff_gpu,
s1_hat_gpu, s2_hat_gpu, s1_gpu, s2_gpu, s_gpu, hyperdiffu_hat_gpu, diffu_gpu, hyperdiffu_gpu, Fsmag, psi_gpu, meanvb1_gpu, meanvb2_gpu, CB_gpu, vb1_gpu, vb2_gpu):

    '''
    ### SMAG to determine hyperdiffusion vt
    ...
    ### Leith to determine hyperdiffusion vt
    ## s_gpu = (w1x_gpu/NX2)*(w1x_gpu/NX2) + (w1y_gpu/NX2)*(w1y_gpu/NX2)

    S_Leith_ker(s_gpu, w1x_gpu, w1y_gpu, block=(TPB,1,1), grid=(HNB,HNX,1))
    eddy_visc_ker(vt_gpu, s_gpu, block=(1,1,1), grid=(1,1,1))
    ...
    multiply_ker(vb1_gpu, psi_gpu, Fsmag, block=(TPB,1,1), grid=(HNB,HNX,1))
    multiply_ker(vb2_gpu, psi_gpu, diffu_gpu, block=(TPB,1,1), grid=(HNB,HNX,1))

    eddy_visc_ker(meanvb1_gpu, vb1_gpu, block=(1,1,1), grid=(1,1,1))
    eddy_visc_ker(meanvb2_gpu, vb2_gpu, block=(1,1,1), grid=(1,1,1))
