...
from leith_eddy_viscosity import *
...
# cs = 0.2#cs# Smag coefficients
    ...
    # Leith coefficients
    # cs = 0.2
    ev_coeff = (cs*Delta)*(cs*Delta)*(cs*Delta)
...
#     Fk = Gk*Fk
    ...
    for it in range(maxit):
        ...
        # SMAG(vt_gpu, kx_gpu, w1_hat_gpu, psiCurrent_hat_gpu, NX, TPB, NB, HNX, HNB, Delta, plan, ev_coeff_gpu)
        leith(vt_gpu, kx_gpu, w1_hat_gpu, psiCurrent_hat_gpu, w1x_gpu, w1y_gpu, NX, TPB, NB, HNX, HNB, Delta, plan, ev_coeff_gpu)
        # vt = vt_gpu.get()
        # print(vt)

        # Construct RHS
        RHS_ker(convec_hat_gpu, diffu_hat_gpu, vt_gpu, w1_hat_gpu, RHS_gpu, block=(TPB,1,1), grid=(NB,NX,1)) 
