...
[convec1_hat, v1_hat] = convection_conserved(psiCurrent_hat, w1_hat, Kx, Ky);
[convec0_hat, v0_hat] = convection_conserved(psiPrevious_hat, w0_hat, Kx, Ky);
diffu_hat = -Ksq.*w1_hat;
[ve,CS] = smag_sgs_calc(w1_hat,psiCurrent_hat,Kx,Ky,Ksq, dx, CS);
% ve = 0; % No SGS
RHS = w1_hat + dt*(-1.5*convec1_hat + 0.5*convec0_hat) + dt*0.5*(nu+ve)*diffu_hat + dt*Fk + dt*beta*v1_hat;
...
%         tempW = max(max(real(ifft(reshape(w1_hat,NX,NX),[],2))))
        ...
        tempPsi = real(ifft2(reshape(psiPrevious_hat,NX,NX)));
        Ene = tempW.*tempPsi;
        Ens = tempW.*tempW;
        Energy = [Energy; mean(Ene(:))];
        Enstrophy = [Enstrophy; mean(Ens(:))];

        slnWor(:,:,count+1) = tempW.';
        SGS(:,:,count+1) = ve*real(ifft2(diffu_hat)).';
...
%     save(['Poislnvor_python', num2str(NX), 'KT.mat'],'psiCurrent','psiPrevious','time','slnW','-v7.3');
    save(['vorticity_field_KT',num2str(Re),'.mat'],'slnWor','SGS','-v7.3');
%     onePLES = slnW;
...
% save('w1DSMAG.mat','w1')
% slnWorDSMAG = slnWor;
% save('w1LES_Re10k_DSMAG.mat','slnWorDSMAG','SGS','slnW','CS','ve','-v7.3');
% save('Energy_LES_DSMAG_10k.mat','Energy','Enstrophy');
